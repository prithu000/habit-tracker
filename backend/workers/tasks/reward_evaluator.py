...
def sync_day_log(self, user_id: str, date_str: str):
    ...
    try:
        ...
        # Count categories where all tasks were completed today
        ...
        for row in Task.objects.filter(user=user, is_active=True).values("category").annotate(n=_Count("id")):
            ...
            if task_count == 0:
                ...
                user=user, task__category=cat, local_date=log_date
            ).count()
            if done >= task_count:
                categories_completed += 1

        # Upsert DayLog (routines_completed field repurposed for categories)
        DayLog.objects.update_or_create(
            ...
            defaults={
                ...
                "xp_earned": xp_earned,
                "routines_completed": categories_completed,
